...
# Configuration
THRESHOLD_PERCENT=30  # Minimum percentage increase to flag as SQLi
...
detect_sqli() {
    ...
    echo "  Original response: $original_count HTML elements"
    echo "  SQL test response: $sql_count HTML elements"
    ...
    if [ -n "$original_title" ] && [ -n "$sql_title" ]; then
        echo "  Original title: $original_title"
        echo "  SQL test title: $sql_title"
    fi
    ...
    if [ "$sql_count" -gt "$original_count" ]; then
        if [ $increase_pct -ge $THRESHOLD_PERCENT ]; then
            echo "[+] ⚠️  POSSIBLE SQL INJECTION VULNERABILITY DETECTED!"
            echo "    URL: $url"
            echo "    Payload: $payload"
            echo "    Element increase: ${increase_pct}% (${original_count} → ${sql_count})"
        ...
        elif [ $increase_pct -gt 10 ]; then
            echo "[?] Suspicious: Moderate element count increase (${increase_pct}%)"
            echo "    Consider manual investigation"
            return 2  # Suspicious
        else
...
check_pattern_changes() {
    ...
    for pattern in "${patterns[@]}"; do
        ...
        if [ "$orig_count" -ne "$sql_count" ]; then
            ...
            echo "      $pattern: ${orig_count} → ${sql_count} (${change})"
        fi
    done
