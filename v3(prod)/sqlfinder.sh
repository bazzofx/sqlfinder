...
while IFS= read -r url; do
  ...
  if [ "$base_code" = "000" ] || [ "$base_code" = "" ]; then
  ...
  if [ "$vulnerable" = false ]; then
    echo -e "${GREEN}[-] No SQLi detected${NC}"
  fi
